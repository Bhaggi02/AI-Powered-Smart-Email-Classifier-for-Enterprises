...
@st.cache_data
def load_data():
    return pd.read_csv("email_categorization_dataset_10000.csv")
...
def preprocess_email_for_category_model(text):
    # Re-using the detailed preprocessing for the category model
    if pd.isna(text) or not str(text).strip():
        return ''
    text = str(text).lower()
...
@st.cache_resource # Cache model training for performance
def train_models(df_data):
    # Apply cleaning for ML models
    df_data['cleaned_email'] = df_data['email'].apply(clean_text)
    df_data['cleaned_email'] = df_data['cleaned_email'].apply(remove_signatures)
    ...
    X_train_cat, X_test_cat, y_train_cat, y_test_cat = train_test_split(X_cat_tfidf, y_category, test_size=0.2, random_state=42, stratify=y_category)

    lr_category_model = LogisticRegression(max_iter=1000, random_state=42)
    lr_category_model.fit(X_train_cat, y_train_cat)

    # Urgency Model Setup
    label_encoder_urgency = LabelEncoder()
    y_urgency = label_encoder_urgency.fit_transform(df_data['urgency'])

    tfidf_urg = TfidfVectorizer() # Use a separate TFIDF for urgency as it might have different features
    X_urg_tfidf = tfidf_urg.fit_transform(df_data['cleaned_email'])
    X_train_urg, X_test_urg, y_train_urg, y_test_urg = train_test_split(X_urg_tfidf, y_urgency, test_size=0.2, random_state=42)
...
# --- Streamlit UI Layout ---
st.set_page_config(layout='wide', page_title="Email Classifier & Urgency Detector")
st.title("✉️ Smart AI Email Classifier & Urgency Detector")
st.write("This application categorizes emails and detects their urgency levels using Machine Learning and keyword-based rules.")
...
user_email = st.sidebar.text_area("Paste an email here for classification:", "")
...
if st.sidebar.button("Classify Email"):
    if user_email:
        ...
        st.success(f"**Predicted Category:** {category_prediction.capitalize()}")
        ...
        st.info(f"**Predicted Urgency:** {final_urgency_prediction_label.capitalize()}")
    else:
        st.sidebar.warning("Please enter an email to classify.")


# --- Visualizations --- 
st.subheader("Dataset and Model Insights")

# Layout for plots
col1, col2 = st.columns(2)

with col1:
    st.write("### Email Category Distribution")
...
with col2:
    st.write("### Email Urgency Distribution")
    ...
    ax_urg.tick_params(axis='x', rotation=45)
    st.pyplot(fig_urg)
...
st.markdown("--- ")
st.markdown("**Note:** For real-time deployment and integration with enterprise systems, specialized cloud services (AWS, GCP, Azure) and API endpoints would be required, as outlined in Milestone 5.")
"""
